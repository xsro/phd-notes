...
第三，考察原点。令 $t=R\cos\theta$、$u=R\sin\theta$，$R\to0^+$。由于
\begin{equation}
 f(t)=\mu_1t+O(t^2),\qquad g(t)=\frac{\mu_1^2}{2}+O(t),
\end{equation}
分子和分母的最低非零阶均为 $R^2$。当 $\sigma=+1$ 且角度趋向 $\theta=\pi/4$ 时，分母二次主项可能消失；但该方向正是 $t=u$，前一步已经证明分子正部在其邻域为零。当 $\sigma=-1$ 时，分母主项为
\begin{equation}
 \mu_1^2R^2(\cos\theta+\sin\theta)^2,
\end{equation}
在 $\theta\in[0,\pi/2]$ 上严格为正。因此原点附近的正部比值有界。

第四，考察无穷远。令同样的极坐标中 $R\to\infty$。此时
\begin{equation}
 f(t)=\mu_2t^2+O(t),\qquad g(t)=\mu_2^2t^2+O(t),
\end{equation}
所以分子和分母的最高阶均为 $R^4$。对 $\sigma=-1$，分母四次主项不为零；对 $\sigma=+1$，唯一可能消失的方向仍为 $t=u$，而在该方向分子的四次主项为负。故无穷远处的正部比值也有界。

第五，去掉原点邻域、无穷远区域以及 $q_+=0$ 的负分子邻域后，剩余参数集合是紧集，且分母连续并严格大于零，因此比值在该紧集上取得有限最大值。综上，式 \eqref{eq:C0-def} 定义的 $C_0$ 有限。
