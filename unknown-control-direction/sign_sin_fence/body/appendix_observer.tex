...
\begin{remark}[上界无界的情形]
若 $\bar n(t)$ 随时间无界增长（即 $\sup_{t\ge0}\bar n(t)=\infty$），则任何固定的 $k_b$ 都会在足够大的 $t$ 处被扰动超过，符号项 $-k_b\sgn(\xi_i)$ 不再能主导 $n_i(t)$，$\dot V$ 的负定性随之丧失，原点不再吸引，系统至多收敛到一个随时间膨胀的残差邻域。此时定常增益设计失效，必须引入随时间增长的增益 $k_b(t)$（例如令 $k_b(t)\rho\ge\bar n(t)$）或采用自适应/增益调度结构，才能恢复有限时间收敛。这已超出本附录的范围。
\end{remark}
