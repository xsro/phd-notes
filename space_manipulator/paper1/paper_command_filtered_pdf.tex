...
\noindent\textbf{关键词}：自由漂浮空间机械臂；周期延迟反馈；预设时间控制；指令滤波反步法；扰动观测器

\vspace{1em}
\noindent\textbf{中图分类号}：TP241\qquad\textbf{文献标志码}：A
\end{abstract}
...
\vspace{0.5em}
\noindent\textbf{论文组织}：第~2~节给出预备知识与问题描述，包括 PDF 基本引理与空间机械臂动力学建模；第~3~节详细阐述控制算法，涵盖 PTDO 设计、计算力矩补偿和指令滤波反步法结合 PDF 的三个核心环节；第~4~节给出稳定性分析与主定理证明；第~5~节讨论鲁棒性与工程实现问题；第~6~节通过七自由度机械臂仿真验证算法有效性；第~7~节总结全文。
...
$\R_{\ge 0}=[0,\infty)$，$\|\cdot\|$ 表示向量的 $2$-范数。$\cC^{r}$ 表示 $r$ 次连续可微函数空间。$h>0$ 为设计延迟；$\vth$ 表示 PDF 控制器的局部时间（相对时间）。

对 $a\ge 0$ 与 $h>0$，$K_{(a,h)}(t)$ 为 PDF 增益函数（见定义~\ref{def:pdf_gain}）；$R_h(t)\in\cS^{(r)}(h)$ 为光滑 $2h$-周期激活函数（见定义~\ref{def:Sh}）。

\begin{definition}[函数类 $\cS^{(r)}(h)$]\label{def:Sh}
...
\begin{definition}[PDF 增益及 Gramian]\label{def:pdf_gain}
给定 $a\ge 0$ 与 $h>0$，定义
\begin{equation}
...
仿真在 MATLAB R2025b 上执行，步长 $T_s^{\text{samp}}=10^{-4}\,\text{s}$。图~\ref{fig:arm} 给出了自由漂浮机械臂的基座平动、末端轨迹与初末构型。由于系统初始动量为零，关节运动通过动量守恒诱导基座产生反作用位移，末端轨迹在基座漂移与关节运动共同作用下完成空间位移。图中右侧的柱状对比显示稳态时刻各关节角度相对于初始值的变化情况。

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{/tmp/sim_results/arm_configuration.png}
\caption{自由漂浮基座平动、末端轨迹与机械臂构型（初末态）}
\label{fig:arm}
\end{figure}

图~\ref{fig:summary} 展示了各关节的位置跟踪、速度跟踪、跟踪误差以及控制力矩的综合响应。七组关节均能在五次多项式参考轨迹作用下平滑变化，轨迹结束后保持有界稳定。$\tanh(100\,\vi_2)$ 替代 $\sign(\vi_2)$ 后控制力矩无明显高频抖振。

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{/tmp/sim_results/joint_tracking_error_torque_summary.png}
\caption{关节位置与速度跟踪、跟踪误差及控制力矩综合响应}
\label{fig:summary}
\end{figure}

图~\ref{fig:norms} 以对数坐标给出了跟踪误差 2-范数的时间历程，其中竖直虚线标记了 PTDO 理论收敛时刻 $T_o$ 和预设时间 $T_o+T_s$。由表~\ref{tab:errors} 可见，在 $t=T_o+T_s=5\,\text{s}$ 时位置误差已降至 $4.71\times10^{-4}\,\text{rad}$，验证了定理~1 的预设时间收敛结论；$t=10\,\text{s}$ 终端位置误差 $9.14\times10^{-4}\,\text{rad}$（$<0.06^\circ$），速度误差 $9.31\times10^{-4}\,\text{rad/s}$。

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{/tmp/sim_results/error_norm_log.png}
\caption{跟踪误差 2-范数（对数坐标，标记 $T_o$ 与 $T_o+T_s$）}
\label{fig:norms}
\end{figure}
...
$t=8\,\text{s}$ 时五次多项式轨迹结束，系统经历 $\ddot q_d$ 跳变，误差出现短暂峰值（$\|e\|_2\approx4.94\times10^{-3}\,\text{rad}$），其后在 $2\,\text{s}$ 内迅速收敛至稳态。稳态包络（$t\ge6\,\text{s}$）为 $\max\|e\|_2=5.89\times10^{-3}\,\text{rad}$、$\max\|\dot e\|_2=3.31\times10^{-3}\,\text{rad/s}$。

图~\ref{fig:torque} 展示了控制力矩的时间历程，最大力矩为 $44.03$ N$\cdot$m，RMS 力矩为 $0.967$ N$\cdot$m（表~\ref{tab:torque}）。力矩曲线光滑无抖振，验证了 $\tanh$ 近似的有效性。

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{/tmp/sim_results/control_torque.png}
\caption{控制力矩时间历程}
\label{fig:torque}
\end{figure}
...
针对捕获前自由漂浮空间机械臂的轨迹跟踪问题，本文提出了一种将指令滤波反步法与光滑周期延迟反馈（PDF）相结合的预设时间控制方案。主要工作总结如下：

(1) 建立了"PTDO 扰动估计 $\to$ 计算力矩补偿 $\to$ 误差线性化 $\to$ 指令滤波 PDF 反步法"的完整架构，将非线性跟踪问题转化为关于补偿误差 $\vi_1,\vi_2$ 的线性延迟镇定问题。

(2) 给出了顺序收敛的严格 Lyapunov 证明：$\vi_2$ 在 $T_o+2h$ 内由强 PDF 镇定，$\vi_1$ 在 $T_o+4h$ 内由线性 PDF 镇定；实际跟踪误差在补偿误差归零后以指数速率收敛到零。

(3) 通过指令滤波避免了 PDF 虚拟控制 $\alpha_1(t)$ 的解析求导，并通过误差补偿系统完全消除了滤波误差，保持了理论的精确性。

(4) 七自由度空间机械臂仿真验证了该方案在预设时间 $T_o+T_s=5\,\text{s}$ 处的收敛精度（$\|e\|_2=4.71\times10^{-4}\,\text{rad}$）和对有界扰动的有效抑制。

\noindent\textbf{未来工作}包括：(i) 将控制律由关节空间扩展至末端任务空间，处理广义雅可比奇异问题；(ii) 引入障碍 Lyapunov 函数处理关节位置和速度约束；(iii) 考虑执行器饱和下的严格预设时间收敛；(iv) 在气浮平台或半物理仿真实验平台上开展硬件在环验证。
