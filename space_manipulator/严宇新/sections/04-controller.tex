...
\]
其中 \(K_p,K_d,K_{ph},K_{dh}\) 为正定对角矩阵，\(\rho>0\) 为延迟反馈幅值系数。该工程化形式仅保留 PDF 的等待--作用周期结构，并不等价于严格的 monodromy matrix 零化增益；论文中的理论固定时间结论仍以式 \eqref{eq:kc_pdf} 的 Gramian 型 PDF 增益为前提。
