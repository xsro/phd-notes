...
设 \(z_1,z_2\in\mathbb R^n\) 分别为 \(\chi\) 和 \(\Delta_a\) 的估计值，取 \(\hat\Delta_a=z_2\)。定义复合误差
\[
\varepsilon_1=\chi-z_1-\bar\xi(t),
\qquad
\varepsilon_2=\Delta_a-z_2-\bar\xi(t),
\]
...
\end{aligned}
\]
在 \(\Delta_a\) 与 \(\dot\Delta_a\) 有界的条件下，观测误差 \(\chi-z_1\) 和 \(\Delta_a-z_2\) 可在 \(t=T_o\) 收敛到零。参数 \(T_o\) 和 \(T_c\) 决定观测收敛时间，但不宜选得过小；否则会带来较大的观测峰值和控制输入峰值。
