...
因为 $\mathcal{A}(t)\to A$ 指数快速，且 $\mathcal{A}(t)$ 有界，
\begin{equation}
\begin{aligned}
C(t)&\to
C_\infty\triangleq
...
将 $C(t)$ 按照 $z=\col(z_1,z_2)$ 分块：
\begin{equation}
\begin{aligned}
\dot{z}_1&=-\varepsilon C_{11}(t)z_1
             -\varepsilon C_{12}(t)z_2+r_1(t),\\
\dot{z}_2&=-\varepsilon C_{21}(t)z_1
             -\varepsilon C_{22}(t)z_2+r_2(t).
\end{aligned}
\tag{C2}
\end{equation}
...
\]
在有限区间 $[t_0,T]$ 上，这是一个系数分段连续的线性系统，不会发生有限时间逃逸，
因此 $z(t)$ 也有界。于是
\begin{equation}
z(t)\ \text{以及}\ x(t)=x^*+Pz(t)\ \text{均有界}.
\tag{B}
\end{equation}
