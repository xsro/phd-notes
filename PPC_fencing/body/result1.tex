...
\eqref{eq:ui}, and its stability and performance properties are established
in \S\ref{sec:analysis}.  Technical limitations and implementation caveats
are discussed in \S\ref{subsec:limits}.
...
\[
u_i-u_0-\phi_i
= \bar u-k_p(p_i-\hat p_i)-k_v(v_i-\dot{\hat p}_i)+\psi_i,
\]
with $\sum_i\psi_i=0$ by Assumption~\ref{as:consensus}, gives
\[
\begin{aligned}
\dot V_c
&= N\bar u^T\dot{\tilde p}
   - k_p\sum_{i} v_{i0}^T(p_i-\hat p_i)\\
&\quad - k_v\sum_{i} v_{i0}^T(v_i-\dot{\hat p}_i)
...
&= -k_v\sum_i\norm{v_{i0}}^2
   - k_v\sum_i v_{i0}^Tv_0 \\
&\quad + k_v\sum_i v_{i0}^T\dot{\hat p}_i .
\end{aligned}
\]
...
and artificial-potential-fence architecture.  It captures the intended
closed-loop mechanism, but several technical points require additional care
in a fully rigorous implementation.
