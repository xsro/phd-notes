...
performance function $\rho(t)>0$ so that the (scalar) enclosure error
$e(t)=\tilde p^{(\ell)}(t)$ of every component $\ell=1,\dots,n$ satisfies
\begin{equation}
    |e(t)| < \rho^{(\ell)}(t), \qquad \forall\, t\ge 0.
    \label{eq:ppc-constraint}
\end{equation}
The function $\rho^{(\ell)}(t)$ is smooth, strictly positive, decreasing,
with $\rho^{(\ell)}(0)>|e(0)|$ and
$\lim_{t\to\infty}\rho^{(\ell)}(t)=\rho_\infty^{(\ell)}>0$ (taking
$\rho_\infty^{(\ell)}=0$ gives asymptotic convergence to zero). It
simultaneously prescribes:
\begin{itemize}
    \item the maximum overshoot: $|e(t)|<\rho^{(\ell)}(0)$;
    \item the convergence rate (via the decay rate of $\rho^{(\ell)}$);
    \item the steady-state error bound $\rho_\infty^{(\ell)}$.
\end{itemize}
Since $\norm{\tilde p(t)}\le \sum_{\ell=1}^n \rho^{(\ell)}(t)$, the
prescribed bound on $\tilde p$ directly bounds the enclosure distance
$\dist(p,p_0)$.

\textbf{Asymptotic (exponential) performance function:}
\begin{equation}
    \rho^{(\ell)}(t)=(\rho_0^{(\ell)}-\rho_\infty^{(\ell)})e^{-\ell^{(\ell)} t}
    +\rho_\infty^{(\ell)},
    \qquad \rho_0^{(\ell)}>|e(0)|,\ \ell^{(\ell)}>0.
    \label{eq:rho-exp}
\end{equation}
...
\begin{equation}
    \rho^{(\ell)}(t)=
    \begin{cases}
    \bar\rho^{(\ell)} + (\rho_0^{(\ell)}-\bar\rho^{(\ell)})
        \bigl(1-\tfrac{t}{T}\bigr)^r, & 0\le t<T,\\[4pt]
    \bar\rho^{(\ell)}, & t\ge T,
    \end{cases}
    \qquad r\ge 2,
    \label{eq:rho-pt}
\end{equation}
which reaches the target value $\bar\rho^{(\ell)}$ exactly at $t=T$.
...
$T(\cdot)=\tanh(\cdot)\in(-1,1)$ and let the normalized error
$z^{(\ell)}=e/\rho^{(\ell)}$ satisfy
\[
z^{(\ell)} = T(\xi^{(\ell)})\quad\Longleftrightarrow\quad
\xi^{(\ell)} = T^{-1}(z^{(\ell)})
= \operatorname{artanh}\!\left(\frac{e}{\rho^{(\ell)}}\right)
= \frac{1}{2}\ln\frac{1+e/\rho^{(\ell)}}{1-e/\rho^{(\ell)}}.
\]
As long as $\xi^{(\ell)}(t)$ stays finite,
$|z^{(\ell)}(t)|=|T(\xi^{(\ell)})|<1$, so \eqref{eq:ppc-constraint} holds
automatically. Differentiating $\xi^{(\ell)}$ (with
$\zeta^{(\ell)}=1/(1-z^{(\ell)2})>0$) gives
\begin{equation}
    \dot\xi^{(\ell)}
    = \frac{\zeta^{(\ell)}}{\rho^{(\ell)}}
      \bigl(\dot e - z^{(\ell)}\dot\rho^{(\ell)}\bigr),
    \qquad z^{(\ell)}=\frac{e}{\rho^{(\ell)}}.
    \label{eq:xi-dyn}
\end{equation}
Thus the fencing problem reduces to designing a controller that keeps every
$\xi^{(\ell)}$ bounded (and drives it to zero)---the standard PPC
backstepping task for the double-integrator centroid dynamics
...
\begin{definition}[PPC fencing property]
    ...
    \begin{enumerate}
        ...
        $| \tilde p^{(\ell)}(t) | \le \rho^{(\ell)}(t)$ for all
        $t\ge 0$ and $\ell=1,\dots,n$, where $\rho^{(\ell)}(t)$ is a
        user-specified performance function of the form \eqref{eq:rho-exp}
        or \eqref{eq:rho-pt}.
    \end{enumerate}
    Property P4 implies P1 (since $\rho^{(\ell)}(t)\to\rho_\infty^{(\ell)}$,
    and $\rho_\infty^{(\ell)}=0$ gives exact enclosure); the novelty is that
    the \emph{transient} behavior of the enclosure is now explicitly bounded
    by the user-chosen envelope.
\end{definition}

\begin{problem}[Prescribed-performance fencing control]
    \label{prob:main}
    Given the double-integrator dynamics
    \eqref{eq:agent}--\eqref{eq:target} and prescribed performance functions
    $\rho^{(\ell)}(t)$, design a \emph{distributed} control law $u_i$ using
    only local information such that the closed loop satisfies:
    \begin{enumerate}
        \item[\textbf{P4}] prescribed-performance enclosure:
        $| \tilde p^{(\ell)}(t) | < \rho^{(\ell)}(t)$, $\forall\, t\ge 0$;
    ...
    below, while P4 prescribes the soft performance envelope on the enclosure
    error.
\end{problem}
