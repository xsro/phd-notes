\subsection{Controller 5-2: Adaptive Robust Gain (Asymptotic)}
\label{sec: controller5-2}
...
term $Y_i\tilde{\Theta}_i$. Controller 5-2 removes this requirement by letting the gain be
...
\begin{equation}\label{eq5-2:control_input}
...
Substituting \eqref{eq5-2:control_input} into \eqref{eq hetero agent} and using
\eqref{eq5-2:regression} gives
\begin{equation}\label{eq5-2:s_dynamics}
...
\begin{equation}\label{eq5-2:V}
	V_{1i} = \frac12 s_i^T M_i s_i
	+ \frac12 \tilde{\Theta}_i^T \Lambda \tilde{\Theta}_i
	+ \frac{1}{2\gamma}\tilde{\rho}_i^2.
\end{equation}
Differentiating and using \eqref{eq5-2:s_dynamics}, \eqref{eq5-2:adaptation},
\eqref{eq5-2:rho_adapt} and the skew-symmetry property A2,
\begin{align}
	\dot{V}_{1i}
	&= -k\|s_i\|^2 - \hat{\rho}_i\|s_i\|_1
		...
		+ \tilde{\rho}_i\|s_i\|_1 \notag\\
	&= -k\|s_i\|^2 - \rho_i^*\|s_i\|_1 \le 0. \label{eq5-2:V_dot}
\end{align}
...
\begin{theorem}
	...
	\eqref{eq5-2:control_input}, \eqref{eq5-2:adaptation} and \eqref{eq5-2:rho_adapt}
...
\noindent\textbf{Proof}
The identity \eqref{eq5-2:V_dot} gives $s_i \to 0$ via Barbalat's lemma, with no PE
...
identically. From \eqref{eq5-2:s_dot_def},
...
\begin{remark}
	The gain $\hat{\rho}_i$ grows monotonically according to \eqref{eq5-2:rho_adapt} and
