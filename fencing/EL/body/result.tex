...
\tR{[unclear about the conditions]}
\textcolor{blue}{please check the algorithm when $\theta_i$ is known}
...
\begin{equation}\label{eq:sliding_var}
	s_i = \dot{q}_i - \zeta_i,
\end{equation}
where $\zeta_i = \dot{q}_0 - k_{\alpha} {q}_{i0} + \int_0^t  -k_{\beta} {q}_{i0} + \phi_i(\tau) d \tau$.
...
\begin{theorem}
	The HELS \eqref{eq hetero agent} governed by \eqref{eq:control_input} and \eqref{eq:regression} can fence the target under Assumptions %具体需要的条件需要辨析。
\end{theorem}
...
\begin{equation}\label{eq:s_dot}
	\dot{s}_i = \ddot{q}_{i0}+k_\alpha \dot{q}_{i0}+ k_\beta  q_{i0}-\phi_i.
\end{equation}
...
%According to Lyapunov stability theory, \( \dot{V_1} \leq 0 \) indicates that the system is globally asymptotically stable, i.e., the auxiliary value \( s_i \to 0 \), and thus the tracking errors \( \tilde{q}_i \to 0 \), \( \dot{\tilde{q}}_i \to 0 \), satisfying the control objective.

\tR{
[$\ddot{V}$ and $Y_i(q_{i},\dot{q}_{i},\dot{\zeta}_i,\zeta_i)$ are difficult to verify boundedness]
...
%, and thus the tracking errors \( \tilde{q}_i \to 0 \), \( \dot{\tilde{q}}_i \to 0 \), satisfying the control objective.
}
...
Considering \(\lim_{t\to\infty}\|s_i\|=0\), we get \(\lim_{t\to\infty}\|\dot{s}_i\|=0\).
From equation \eqref{eq:s_dot}, we have:
\begin{equation}\label{eq:q0_ddot}
	\ddot{q}_{i0}=-k_\alpha \dot{q}_{i0} -k_\beta {q}_{i0} + \phi_i + \dot{s}_i.
\end{equation}
...
\begin{equation*}\label{eq:phi_i}
	\phi_i= \sum_{i\in\mathcal{N}}{\alpha}({\left\| q_{ij} \right\|})\frac{q_{ij}}{\left\| q_{ij} \right\|},
\end{equation*}
then we simplify to
\begin{equation*}\label{eq:V2_dot_final}
	\dot{V_2} =\dot{q}_{i0}^T\bigl(-k_\alpha \dot{q}_{i0} + \dot{s}_i\bigr)
	=-k_\alpha \dot{q}_{i0}\dot{q}_{i0}^T + \dot{q}_{i0}^T\dot{s}_i
	=-k_\alpha\left\| \dot{q}_{i0} \right\|^2+\dot{q}_{i0}^T\dot{s}_i.
\end{equation*}

Let
\begin{equation*}\label{eq:h_t}
	h(t)=\dot{q}_{i0}^T\dot{s}_i,
\end{equation*}
\begin{equation*}\label{eq:V2_dot_h}
	\dot{V_2} =-k_\alpha\left\| \dot{q}_{i0} \right\|^2+h(t),
\end{equation*}
\begin{equation*}\label{eq:W}
	W=V_2-\int_0^t h(\tau) \, d\tau.
\end{equation*}
%Since \( \dot{s}_i \to 0 \), i.e., it converges to 0 in finite time, the
Since \( \dot{s}_i \to 0 \), i.e., it converges to 0 asymptotically, the 
integral \(\int_0^t h(\tau) \, d\tau\) exists, and thus \(W\) is lower bounded.

From the above equation,
\begin{equation*}\label{eq:W_dot}
	\dot{W} =-k_\alpha\left\| \dot{q}_{i0} \right\|^2\leq 0.
\end{equation*}
According to the invariance principle,
\(\dot{q}_{i0} \to 0 \)
that is,
\(\dot{q_i}  - \dot{q}_0 \to 0 \), the velocity of the multi-agent system will converge to the target velocity.

Therefore, property (P3) is proved.



%here start 关于合围凸包P1的证明
% 基于文献第3节Main Results中P1的证明过程
\subsection*{Proof of (P1)}

Define
%\begin{equation}
%	\tilde{q_i}=\frac{1}{N}\sum_{i\in\mathbb{N}}q_{i0},
%\end{equation}
%\begin{equation}
%	\dot{\tilde{q_i}}=\frac{1}{N}\sum_{i\in\mathbb{N}}\dot{q}_{i0},
%\end{equation}
%\begin{equation}
%	\ddot{\tilde{q_i}}=\frac{1}{N}\sum_{i\in\mathbb{N}}\ddot{q}_{i0},
%\end{equation}
\begin{equation*}\label{eq:q_tilde}
	\tilde{q}=\frac{1}{N}\sum_{i\in\mathbb{N}}q_{i0},
\end{equation*}
\begin{equation*}\label{eq:q_tilde_dot}
	\dot{\tilde{q}}=\frac{1}{N}\sum_{i\in\mathbb{N}}\dot{q}_{i0},
\end{equation*}
\begin{equation*}\label{eq:q_tilde_ddot}
	\ddot{\tilde{q}}=\frac{1}{N}\sum_{i\in\mathbb{N}}\ddot{q}_{i0},
\end{equation*}
...
\begin{equation*}\label{eq:q_tilde_ddot_final}
	\ddot{\tilde{q}}=-k_\alpha \dot{\tilde{q}} -k_\beta\tilde{q}+\frac{1}{N}\sum_{i\in\mathbb{N}}\dot{s}_i+\frac{1}{N}\sum_{i\in\mathbb{N}}\phi_{i}.
\end{equation*}

As $j\in\mathcal{N}_{i}\Leftrightarrow i\in\mathcal{N}_{j}$ and $q_{ij}=-q_{ji}$, we have
\begin{equation*}\label{eq:sum_phi}
	\sum_{i\in\mathbb{N}}\phi_{i}=0.
\end{equation*}
Then
\begin{equation*}\label{eq:q_tilde_ddot_simplified}
	\ddot{\tilde{q}}=-k_\alpha \dot{\tilde{q}} -k_\beta\tilde{q}+\frac{1}{N}\sum_{i\in\mathbb{N}}\dot{s}_i.
\end{equation*}

This equation is input-state stable (ISS) as $k_{\alpha}>0$ and $k_{\beta}>0$. According to Step 1, \( \dot{s}_i \to 0 \). It implies that $\tilde{q}$ converges to zero as $t\to\infty$, i.e.,
\begin{equation*}\label{eq:q_tilde_limit}
	\lim_{t\to\infty}\tilde{q}(t)=0.
\end{equation*}
It, together with the facts that
\begin{equation*}\label{eq:q_tilde_meaning}
	\tilde{q}=\frac{1}{N}\sum_{i\in\mathbb{N}}q_{i0}=\frac{1}{N}\sum_{i\in\mathbb{N}}(q_{i}-q_{o}),
\end{equation*}
indicates that
\begin{equation*}
	\lim_{t\to\infty}P_{q_{o}}(q)(t)=0.
\end{equation*}
Thus, (P1) is proved.

\hfill$\square$

Finally, contradiction is used to prove (P2). If the collision between agents $i$ and $j$ occurs, then $\exists T_0 > 0$ such that $\|q_i(T_0) - q_j(T_0)\| = d$ at the first time for $i \neq j \in \mathbb{N}$. As such
...
which implies a contradiction with the boundedness of $W$. Thus, the collision between any pair of agents is avoided and (P2) is proved. This completes the proof.
