...
\begin{theorem}
	The HELS \eqref{eq hetero agent} governed by \eqref{eq:control_input} and \eqref{eq:regression} can fence the target under Assumptions A1--A3 and the persistently exciting (PE) condition.
\end{theorem}
...
Moreover, since $\dot{V}_1 = -k\|s_i\|^2$, we have $s_i \in \mathcal{L}_2 \cap \mathcal{L}_\infty$. From the closed-loop dynamics, $\dot{s}_i$ can be shown to be uniformly bounded, which by Barbalat's lemma implies $\dot{s}_i \to 0$ as $t\to\infty$.
...
\begin{equation*}\label{eq:h_t}
	h(t)=\sum_{i\in\mathcal{N}}\dot{q}_{i0}^T\dot{s}_i,
\end{equation*}
\begin{equation*}\label{eq:V2_dot_h}
	\dot{V_2} =-k_\alpha\sum_{i\in\mathcal{N}}\left\| \dot{q}_{i0} \right\|^2+h(t),
\end{equation*}
...
\begin{equation*}\label{eq:W_dot}
	\dot{W} =-k_\alpha\sum_{i\in\mathcal{N}}\left\| \dot{q}_{i0} \right\|^2\leq 0.
\end{equation*}
...
This equation is input-state stable (ISS) as $k_{\alpha}>0$. According to Step 1, \( \dot{s}_i \to 0 \). It implies that $\tilde{q}$ converges to zero as $t\to\infty$, i.e.,
...
indicates that
\begin{equation*}
	\lim_{t\to\infty}\operatorname{dist}\bigl(q_0(t),\operatorname{co}(q(t))\bigr)=0,
\end{equation*}
